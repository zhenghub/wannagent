...
object ServerSide {
  ...
  class ClientsConnection(remote: ActorRef) extends Actor with ActorLogging with Stash {
    ...
    def clientMan: Actor.Receive = {
      ...
      case PeerClosed =>
        context.stop(self)
      case e =>
        log.info(s"unkown message for ClientsConnection: ${e}")
    }
  ...
  class AddrManager {
    ...
    def closeClient(client: ActorRef): Unit = {
      for (hostId <- hostId2Client.find(_._2.equals(client)).map(_._1);
           servers <- hostId2addr.get(hostId);
           pm <- servers;
           server <- addr2Server.get(pm.localAddr)) {
        addr2Server -= pm.localAddr
        hostId2Client -= hostId
        server ! Close
      }
    }
  ...
  class Server(hostId: String, clientHost: String, clinetPort: Int, addr: InetSocketAddress) extends Actor with ActorLogging {
    ...
    override def receive: Receive = {
      ...
      case Close =>
        context.stop(self)
        log.info(s"shut server at ${addr}")
      case msg =>
        log.warning(s"unkown message ${msg}")
    }
  }
